...
# phase I stratification: response-based
...
rkd_ext_opt_ignoreV.f <- function(k, n2samp, qzi, phaseI_strat){
  ...
  # phase2_id = c(indV0[order_resi0[1:best_k]], indV0[order_resi0[(nV0-best_k+1):nV0]], 
  #               indV1[order_resi1[1:(n2samp/2-best_k)]], indV1[order_resi1[(nV1-(n2samp/2-best_k)+1):nV1]])
  phase2_id = dt_ext$id[ord_qzi[c(1:(n2samp-best_k), (n-best_k+1):n)]]
  mart.opt = qzi[phase2_id]
  best_var =var(mart.opt) #(var(mart.opt[which(simZ.opt == 0)])*sum(simZ.opt==0)+var(mart.opt[which(simZ.opt == 1)])*sum(simZ.opt==1))/n2samp
  
  for (k in (best_k+1):(n2samp-best_k))
  {
    # phase2_id = c(indV0[order_resi0[1:k]], indV0[order_resi0[(nV0-k+1):nV0]], 
    #               indV1[order_resi1[1:(n2samp/2-k)]], indV1[order_resi1[(nV1-(n2samp/2-k)+1):nV1]])
  
    phase2_id = dt_ext$id[ord_qzi[c(1:(n2samp-k), (n-k+1):n)]]
    mart.opt = qzi[phase2_id]
    tmp_var = var(mart.opt)#(var(mart.opt[which(simZ.opt == 0)])*sum(simZ.opt==0)+var(mart.opt[which(simZ.opt == 1)])*sum(simZ.opt==1))/n2samp
    if (tmp_var > best_var) {
      best_k = k
  ...
  # s_id = c(indV0[order_resi0[1:best_k]], indV0[order_resi0[(nV0-best_k+1):nV0]], 
  #          indV1[order_resi1[1:(n2samp/2-best_k)]], indV1[order_resi1[(nV1-(n2samp/2-best_k)+1):nV1]])
  s_id = dt_ext$id[ord_qzi[c(1:(n2samp-best_k), (n-best_k+1):n)]]
  #s_id = dt_ext$id[s_id]
...
rkd_ext_opt_ignoreV_2st.f <- function(k, n2samp, qzi, phaseI_strat, s_id.a=NULL){
  ...
  if(!is.null(s_id.a)){
    ...
    ord_qzi_b = order(qzi_b)

    best_k = k
    
    phase2_id_b <- dt_ext_b$id[ord_qzi_b[c(1:(n2samp-best_k), (nb-best_k+1):nb)]]
    
    phase2_id=c(phase2_id_b, s_id.a)
    mart.opt = qzi[phase2_id]
    #simZ.opt = dt_ext$v[phase2_id]
    best_var = (var(mart.opt))#/n2samp
    
    for (k in (best_k+1):(n2samp-best_k))
    {
      
      phase2_id_b <- dt_ext_b$id[ord_qzi_b[c(1:(n2samp-k), (nb-k+1):nb)]]
      phase2_id=c(phase2_id_b, s_id.a)
      mart.opt = qzi[phase2_id]
      #simZ.opt = dt_ext$v[phase2_id]
      tmp_var = (var(mart.opt))#(var(mart.opt[which(simZ.opt == 0)])*sum(simZ.opt==0)+var(mart.opt[which(simZ.opt == 1)])*sum(simZ.opt==1))/n2samp
    ...
    s_id_b=dt_ext_b$id[ord_qzi_b[c(1:(n2samp-best_k), (nb-best_k+1):nb)]]
    s_id=c(s_id_b, s_id.a)
    ...
    s_id_b=dt_ext$id[ord_qzi[c(1:(n2samp-best_k), (n-best_k+1):n)]]
...
designPhII_resid <- function(phaseI_strat, n2samp, design="optn-Smu1",  #h0.model=list("wei"), 
                             para.H0){
  
  qzi <- Smu_vec.f(phaseI_strat$dt_ext$del, phaseI_strat$dt_ext$time, phaseI_strat$dt_ext$v, para.H0)
  
  if(design %in% c("optn-Smu1")){
    k = min((round(n2samp/2)), sum(del))
    res <- rkd_ext.f(k, n2samp, qzi[[1]], phaseI_strat)
  }else if(design %in% c("optn-Smu2")){
    res <- rkd_ext.f(round(n2samp/2), n2samp, qzi[[2]], phaseI_strat)
  }
